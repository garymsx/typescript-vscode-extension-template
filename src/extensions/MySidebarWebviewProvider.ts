...
export class MySidebarWebviewProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    //this.webviewView!.webview.html = this._getHtmlForWebview(this.webviewView!.webview, this.extensionUri);
    
  }

  /**
   * WebViewで表示するHTMLを生成する
   * @returns 
   */
  getHtmlForWebview(): string {
    ...
    const scriptUri = this.getUri("out/scripts", "index.js");
    const codiconsUri = this.getUri('node_modules', '@vscode/codicons', 'dist', 'codicon.css');
  ...
  */
  private getUri(...pathList: string[]): vscode.Uri {
    return getUri(this.webviewView!.webview, this.extensionUri, pathList);
  }
}